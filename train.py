...
from pace_clrf.engine import run_training
...
def main() -> None:
    parser = build_common_parser("Distributed training for PACE-CLRF")
    ...
    try:
        ...
        if is_main_process():
            ...
            print(
                f"[PACE] enabled={model.use_pace}, "
                f"strength={config['model']['pace_strength']}"
            )
            print(
                f"[CLRF] retain_ratio={config['model']['clrf_retain_ratio']}"
            )
